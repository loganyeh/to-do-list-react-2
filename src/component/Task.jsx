...
function Task({id, description}) {
    const {setTasks, taskCount, setTaskCount, lineThrough, setLineThrough} = useContext(MyContext);
    ...
    const handleLineThrough = (id) => {
        setLineThrough(prev => id ? `line-through` : "");
    ...
    return (
        <>
             <div className="h-24 w-1/2 flex flew-row mt-5">
                ...
                <button onClick={() => handleDelete(description)} id={taskCount} className="h-full w-1/3 font-semibold text-black text-xl border-2 border-l-0 border-red-600 hover:bg-gray-50 active:bg-gray-200">Delete</button>
            </div>
        </>
    )
}
